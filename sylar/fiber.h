...
namespace sylar
{
    ...
    public:
        ...
        /**
         * @brief 协程状态
         * @details 在sylar的基础上进行了状态简化，只定义三态转换关系，协程要么正在运行(RUNNING)，
    ...
    public:
         ...
         * @param[in] run_in_shceduler 本协程是否参与调度器调度，默认为true
         */
    ...
    public:
         ...
         * 且该协程为当前线程的主协程，其他协程都通过这个协程来调度，也就是说，当其他协程结束时，
         * 都要切回到主协程，由主协程重新选择新的协程进行resume
         * @attention 线程如果要创建协程，那么应该首先执行一下Fieber::GetThis()操作，以初始化主函数协程
         */
